...
export default function AboutPage() {
  ...
  const content = {
    KR: {
      ...
      sect1Body: "작은 앱이나 인디 게임을 개발하고 UI 프로토타입을 제작할 때, 배경이 제거된 움직이는 에셋이 필요한 경우가 많습니다. 크로마키 녹화를 하거나 AI를 활용해 짧은 모션 클립을 생성하곤 하죠. 문제는 수십 개의 프레임 전체에서 단색 배경을 정리하는 작업이 무척 번거롭다는 점입니다. 전문 비디오 편집 소프트웨어는 무겁고 비싸며, 단순한 스프라이트 시트 하나를 추출하기엔 과도한 기능을 가지고 있습니다. BananaCut은 오직 개발자와 창작자분들이 이 구체적인 문제를 해결할 수 있도록 돕는 경량화 툴입니다.",
      sect2Title: "브라우저 기반 처리의 강점",
      sect2Body: "BananaCut의 미디어 편집 처리는 브라우저 안에서 진행됩니다. 원본 미디어 파일과 편집 결과물을 BananaCut 서버에 전송하거나 저장하지 않습니다. 분석, 광고, 후원 및 외부 링크는 사용자의 선택과 각 제3자 서비스 정책에 따라 별도로 작동할 수 있습니다.",
    ...
    EN: {
      ...
      sect1Body: "When creating small apps, indie games, or prototyping UI, you often need isolated moving assets. You either record something on a green screen or generate a quick motion clip using AI. The problem is removing that solid background across dozens of frames. Professional video editing software is heavy, expensive, and overkill for extracting a simple sprite sheet. BananaCut is a lightweight tool specifically for developers and creators to solve this narrow problem quickly.",
      sect2Title: "Why browser-based processing matters",
      sect2Body: "BananaCut processes media editing tasks in your browser. Your original media files and editing results are not uploaded to or stored on BananaCut servers. Analytics, advertising, donations, and external links may operate separately according to user choices and the policies of each third-party service.",
    ...
    JP: {
      ...
      sect1Body: "小規模なアプリやインディーズゲームの開発、UI의 프로토타입 작성時には、背景が透過されたアニメーション素材が必要になることがよくあります。グリーンバックで録画したり、AIで短いモーションクリップを作成したりする手法です。しかし、数十枚に及ぶフレームから単一色背景を取り除く作業は極めて手間がかかります。プロ向けの動画編集ソフトウェアは重く、高価で、単純なスプライトシートを抽出するにはオーバースペックです。BananaCutは、開発者やクリエイターがこの特定の課題を解決できるよう設計された軽量ユーティリティです。",
      sect2Title: "ブラウザ内処理にこだわる理由",
      sect2Body: "BananaCutのメディア編集処理はブラウザ内で行われます。元のメディアファイルや編集結果がBananaCutのサーバーに送信または保存されることはありません。分析、広告、スポンサーシップ、および外部リンクは、ユーザーの選択や各サードパーティサービスのポリシーに従って個別に動作する場合があります。",
