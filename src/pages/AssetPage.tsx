...
import { generateSpriteSheet } from "../utils/spriteSheetEngine";
...
export default function AssetPage() {
  ...
  const executeExportSprite = async () => {
    ...
    try {
      const { blob, metadata, warning } = await generateSpriteSheet({
        frames: frames.map((f, idx) => ({
          name: f.name || `frame_${idx.toString().padStart(4, "0")}`,
          url: getFrameDisplayUrl(f, 'final'),
        })),
        columns,
        spacing,
        exportSizeMode,
      ...
      if (blob) {
        setSpriteUrl(URL.createObjectURL(blob));
